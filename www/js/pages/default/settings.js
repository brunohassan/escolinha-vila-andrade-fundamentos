...
settingsPage.natificationstatusList = [
    { id: "on", name: "On" },
...
settingsPage.createInDefaultViewAndShow = function() {
    ...
    if (settingsPage.ID != defaultView.getLastOpenedPage().ID) {
        ...
        // UI SELECT TEXT: natifications.
        box.natificationsGroup = settingsPage.createUISelectTextWithTitle("natifications")
        box.add(that)
        that.uiSelectText.id = "natifications"
        that.uiSelectText.createItemsByDataList(settingsPage.natificationstatusList)
        that.uiSelectText.onChange(settingsPage.printSelectedName)
        that.uiSelectText.setSelectedIndex(global.settings[that.uiSelectText.id + "Index"])
